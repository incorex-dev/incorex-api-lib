...
func api_query(method string, params ApiParams) (ApiResponse, error) {
    ...
    req, _ := http.NewRequest("POST", "https://api.incorex.com/v1/"+method, bytes.NewBuffer([]byte(post_content)))
...
func main() {
    ...
    result1, err1 := api_query("user_trades", ApiParams{"pair": "BTC_USD", "limit": 100, "offset": 0})
    if err1 != nil {
        fmt.Printf("api error: %s\n", err1.Error())
    } else {
        fmt.Printf("api result: %v\n", result1)
